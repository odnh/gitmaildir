...
do
   ...
   { time -p seq -f "data_75kB/file.%05g" 0 $i | tr '\n' '\0' | xargs -P1 -n1 -I% -0 sh -c 'cat % | ../../gitmaildir/_build/default/evaluation/performance_test/maildir_cli.exe deliver --dir=gmd' ; } 2>&1 | grep real | sed 's/real //' | awk 1 ORS='' >> log.txt
